...
function SectionHeader({ children, colSpan }: { children: React.ReactNode; colSpan?: number }) {
  return (
    <tr><td colSpan={colSpan || 10} className="px-3 py-2 text-xs font-bold text-white uppercase tracking-wider bg-[#0049B8]">{children}</td></tr>
  )
}

function SectionLabel({ children, colSpan }: { children: React.ReactNode; colSpan?: number }) {
  return (
    <tr><td colSpan={colSpan || 10} className="px-3 pt-4 pb-2 text-xs font-bold text-[#0049B8] uppercase tracking-wider border-b border-gray-200 bg-white">{children}</td></tr>
  )
}
...
function DashboardTab() {
  return (
    <div className="p-4 space-y-6">
      {/* Deal Summary Bar */}
      <div className="bg-[#0049B8] rounded-xl p-4 text-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={m.label} className="border border-gray-200 rounded-xl overflow-hidden">
            <div className="bg-[#0049B8] px-3 py-1.5">
              <p className="text-[10px] text-blue-100 uppercase tracking-wider">{m.label}</p>
            </div>
            <div className="px-3 py-3">
              <p className={`text-xl font-bold font-mono ${m.accent ? "text-[#0049B8]" : "text-gray-900"}`}>{m.value}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[900px]">
        <thead>
          <tr>
            <th className="px-3 py-2 text-left text-sm font-bold text-white bg-[#0049B8] min-w-[200px]">Line Item</th>
            {yy.map(y => (
              <th key={y.year} className="px-3 py-2 text-right text-sm font-bold text-white bg-[#0049B8] min-w-[100px]">Year {y.year}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {/* Income Section */}
          <tr><td colSpan={8} className="px-3 pt-3 pb-1 text-xs font-bold text-[#0049B8] uppercase tracking-wider bg-blue-50/50 border-b border-gray-200">Income</td></tr>
          ...
          {/* Expense Section */}
          <tr><td colSpan={8} className="px-3 pt-3 pb-1 text-xs font-bold text-[#0049B8] uppercase tracking-wider bg-blue-50/50 border-b border-gray-200">Operating Expenses</td></tr>
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[600px]">
          <thead>
            <tr>
              <th className="px-3 py-2 text-sm font-bold text-white bg-[#0049B8] text-left">IRR</th>
              {s.exitCapRates.map(cap => (
                <th key={cap} className={`px-4 py-2 text-sm font-bold text-white text-center ${cap === 7.5 ? "bg-[#003a93]" : "bg-[#0049B8]"}`}>
                  {cap}% Exit Cap
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {s.rentGrowthRates.map((rg, ri) => (
              <tr key={rg}>
                <td className={`px-3 py-3 text-sm font-semibold border-b border-gray-200 ${rg === 3.0 ? "bg-[#0049B8] text-white" : "bg-gray-50 text-gray-700"}`}>
                  {rg}% Rent Growth
                </td>
                {s.matrix[ri].map((irr, ci) => {
                  ...
                  return (
                    <td key={ci} className={`px-4 py-3 text-center text-sm font-bold font-mono border-b border-gray-200 ${isBase ? "ring-2 ring-[#0049B8] ring-inset " : ""}${color}`}>
                      {irr.toFixed(1)}%
                    </td>
                  )
                })}
      ...
      <div className="flex items-center gap-4 text-xs text-gray-400 mt-2">
        ...
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded ring-2 ring-[#0049B8]" /> Base Case</span>
      </div>
    </div>
  )
}
    ...
    <div className="p-4 space-y-6">
          ...
          <tbody>
            {w.yearlyDistributions.map(y => (
              <tr key={y.year}>
                ...
                <td className="px-3 py-2 text-sm text-[#0049B8] font-medium font-mono text-right border-b border-gray-100">{fmt(y.excessToSponsor)}</td>
      ...
      <div className="bg-[#0049B8]/5 border border-[#0049B8]/20 rounded-xl p-4">
        ...
        <div className="space-y-1.5 text-sm">
          ...
          <div className="flex justify-between text-[#0049B8]"><span>Sponsor Share</span><span className="font-bold font-mono">{fmt(w.exitProceeds.sponsorShare)}</span></div>
