            ...
            return (
              <div key={label} className="flex items-center gap-2 flex-1">
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                  done ? "bg-emerald-100 text-emerald-700" : active ? "bg-[#0049B8] text-white" : "bg-gray-100 text-gray-400"
                }`}>
          ...
          {stage === "upload" && (
            ...
            >
              <div
                ...
                className="border-2 border-dashed border-gray-300 hover:border-[#0049B8] rounded-2xl p-12 text-center cursor-pointer transition-all hover:bg-[#0049B8]/5 group"
              >
                <div className="w-16 h-16 rounded-2xl bg-gray-100 group-hover:bg-[#0049B8]/10 flex items-center justify-center mx-auto mb-4 transition-colors">
                  <Upload className="w-8 h-8 text-gray-400 group-hover:text-[#0049B8] transition-colors" />
                </div>
                ...
                <button className="px-6 py-2.5 bg-[#0049B8] text-white rounded-lg text-sm font-medium hover:bg-[#003d99] transition-colors">
                  Upload Sample OM
                </button>
            ...
            >
              <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-12 h-12 rounded-xl bg-[#0049B8]/10 flex items-center justify-center">
                    <BrainCircuit className="w-6 h-6 text-[#0049B8] animate-pulse" />
                  </div>
                  <div>
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="text-[#0049B8] font-medium">{Math.round(processingProgress)}%</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-gray-100 overflow-hidden">
                    <motion.div
                      className="h-full rounded-full bg-gradient-to-r from-[#0049B8] to-blue-400"
                ...
                <div className="space-y-2">
                  {underwritingDeal.processingSteps.map((step, i) => (
                    <div key={i} className={`flex items-center gap-3 text-sm transition-all ${
                      i < processingStep ? "text-emerald-600" : i === processingStep ? "text-[#0049B8] font-medium" : "text-gray-300"
                    }`}>
                      {i < processingStep ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="text-right">
                      ...
                      <span className="font-bold text-[#0049B8]">{underwritingDeal.summary.irr}%</span>
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] text-gray-400 uppercase block">Equity Multiple</span>
                      <span className="font-bold text-[#0049B8]">{underwritingDeal.summary.equityMultiple}x</span>
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] text-gray-400 uppercase block">Cash-on-Cash</span>
                      <span className="font-bold text-[#0049B8]">{underwritingDeal.summary.cashOnCash}%</span>
                    </div>
                  </div>
                </motion.div>
              )}
