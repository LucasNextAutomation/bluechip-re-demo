...
const stageConfig: Record<PipelineStage, { label: string; color: string; bgColor: string }> = {
  ...
  meeting_set: { label: "Meeting Set", color: "text-[#0049B8]", bgColor: "bg-blue-50 border-[#0049B8]/30" },
}
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
          {[
            { label: "Total Leads", value: outreachStats.totalLeads.toString(), icon: Users, color: "text-[#0049B8]" },
                    ...
                    {leads.map(lead => (
                      <motion.div
                        ...
                        className={`bg-white border rounded-xl p-3 cursor-pointer hover:shadow-md transition-all ${
                          selectedLead?.id === lead.id ? "border-[#0049B8] shadow-md" : "border-gray-200"
                        }`}
                      >
                  ...
                  {selectedLead.meetingDate && (
                    <div className="bg-[#0049B8]/5 border border-[#0049B8]/20 rounded-xl p-4">
                      <div className="flex items-center gap-2">
                        <Calendar className="w-5 h-5 text-[#0049B8]" />
                        <div>
                          <h4 className="text-sm font-bold text-[#0049B8]">Meeting Scheduled</h4>
                      ...
                      {emailSequence.map((email, i) => {
                        ...
                        return (
                          <div key={i} className={`border rounded-xl p-4 transition-all ${
                            isCurrent ? "border-[#0049B8] bg-[#0049B8]/5" : isActive ? "border-emerald-200 bg-emerald-50/50" : "border-gray-200 bg-gray-50 opacity-60"
                          }`}>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
